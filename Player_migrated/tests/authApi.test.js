...
it('never persists the signup password and sends verification only after storing the profile', async () => {
  const result = await register({ first_name: 'First', last_name: 'Last', email: ' P@EXAMPLE.TEST ', password: 'secret-fixture' })
  const [path, record] = setDoc.mock.calls[0]
  expect(path).toBe('users/p')
  expect(record).toMatchObject({ email: 'p@example.test', displayName: 'First Last', role: 'player', teamId: null, manuallyVerified: false })
...
it('does not send verification if profile creation fails', async () => {
  ...
  await expect(register({ first_name: 'F', last_name: 'L', email: 'p@example.test', password: 'fixture' })).rejects.toMatchObject({ code: 'AUTH_ERROR' })
